...
class Session {
	...
	public static function get($session_id) {
		$s = \Ex\Redis::get('session:'.$session_id);
		if ($s) {
	...
	public static function set($session_id, $session, $ttl=null) {
		$s = \Ex\Redis::set('session:'.$session_id, json_encode($session), $ttl);
	}

	// 刷新session时间
	public static function refresh($session_id, $ttl=null) {
		$s = \Ex\Redis::setTimeout('session:'.$session_id, $ttl);
	}

	// 清session
	public static function clear($session_id) {
		$s = \Ex\Redis::del('session:'.$session_id);
	}
}
